...
class FiloArac(db.Model):
    """Şirket filo araçları — filo_arac_master tablosu (salt okunur)."""
    __tablename__ = "filo_arac_master"

    id              = db.Column(db.BigInteger, primary_key=True)
    plaka           = db.Column(db.String(20), index=True)
    plaka_durum     = db.Column(db.String(50), index=True)
    marka           = db.Column(db.String(100), index=True)
    seri            = db.Column(db.String(100), index=True)
    model           = db.Column(db.Text)
    model_yili      = db.Column(db.SmallInteger)
    renk            = db.Column(db.String(50))
    yakit_tipi      = db.Column(db.String(50))
    vites_tipi      = db.Column(db.String(50))
    cekis_tipi      = db.Column(db.String(50))
    son_km          = db.Column(db.Integer)
    son_km_tarihi   = db.Column(db.DateTime)
    alis_fiyati     = db.Column(db.Numeric)
    alis_tarihi     = db.Column(db.Date)
    hasar_adedi     = db.Column(db.Integer)
    son_hasar_tarihi = db.Column(db.DateTime)
    son_hasar_sekli = db.Column(db.String(100))
    son_hasar_durum = db.Column(db.String(100))
    pert_var_mi     = db.Column(db.Boolean)
    toplam_hasar_gider     = db.Column(db.Numeric)
    toplam_hasar_gelir     = db.Column(db.Numeric)
    sigorta_tahsilat_toplam = db.Column(db.Numeric)
    hasar_ozeti     = db.Column(db.JSON)
    kaynak_dosya_tarihi = db.Column(db.Date)
    ...
    def to_dict(self):
        return {
            ...
            "cekis_tipi":  self.cekis_tipi,
            "son_km":      self.son_km,
            "alis_fiyati": float(self.alis_fiyati) if self.alis_fiyati else None,
            "alis_tarihi": self.alis_tarihi.isoformat() if self.alis_tarihi else None,
            "hasar_adedi": self.hasar_adedi,
        }
